...
def winState(state, player):
    ...
    if [player, player, player, player] in win_state_medium_board:
        return True
    else:
        return False
...
def minimax(state, depth, player, expanded):
    ...
    for cell in availablePositions(state):
        x, y = cell[0], cell[1]
        state[x][y] = player
        score = minimax(state, current_depth - 1, changePlayer(player), expanded)
        state[x][y] = 0
        score[0], score[1] = x, y
...
if __name__ == '__main__':
    print("Minimax Tic-Tac-Toe Board 4x4 Board")
    main()
